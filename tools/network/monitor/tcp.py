...
"""
import sys, time, os, json,commands,re
reload(sys)
...
def TcpMonitor(TmIp,TmPort):
    try:
        Tmping = Ping(TmIp, TmPort, interval=0.1, timeout=1)
        res = Tmping.ping(4)   # 定义发起请求的次数
        if str(res['maximum']) == '0.0':
            return 'timeout'
        else:
            return res['maximum']   #返回最大值
    except:
        return 'timeout'


print TcpMonitor('192.168.2.1',22)
